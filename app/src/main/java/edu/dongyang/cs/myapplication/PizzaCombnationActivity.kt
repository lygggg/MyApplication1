...
import kotlinx.android.synthetic.main.cheazepizza.*
import kotlinx.android.synthetic.main.combinationpizza.*

class PizzaCombnationActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.combinationpizza)
        var price: Int = 12500
        pizza_plus_colla.setOnCheckedChangeListener { compoundButton, b ->
            if (pizza_plus_colla.isChecked == true) {
                price += 2000
                pizza_combination_price.text = "가격: "+price
            } else
                price -= 2000
            pizza_combination_price.text = "가격: "+price
